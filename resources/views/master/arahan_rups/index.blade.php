            ...
            <div class="content-header row">
            	<div class="content-header-left col-md-6 col-xs-12 mb-2">
            		<h3 class="content-header-title mb-0">Manajemen Arahan RUPS</h3>
            			...
            			<div class="col-xs-12">
            				<div class="card">
            					<div class="card-header">
            						<h4 class="card-title">Daftar Arahan RUPS</h4>
            						<a class="heading-elements-toggle"><i class="fa fa-ellipsis-v font-medium-3"></i></a>
            						<div class="row mt-1">
                                   		<div class="col-md-12 col-xl-3">
                                   			<a href="#" class="btn btn-success btn-sm" data-target="#tambah" data-toggle="modal"><i class="fa fa-plus"></i> Tambah Arahan RUPS</a>
                                   		</div>
                                   	</div>
            					</div>
            							...
            							<div class="modal-content">
            								<div class="modal-header">
            									<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><center>
            										<center><h4 class="modal-title text-success" id="myModalLabel" ><i class="fa fa-plus"></i> Tambah Arahan RUPS</h4></center>
            									</div>
            									<form enctype="multipart/form-data" role="form" action="{{ URL('arahan_rups/store_arahan_rups') }}" method="POST" >
            										{{ csrf_field() }}
            										<div class="modal-body">

            											<label class="control-label"><b> Arahan RUPS </b></label>
            											<label class="control-label"> : </label>
            											<input class="form-control" type="text" name="arahan_rups" placeholder="masukkan arahan RUPS" required="required"/>
            									...
            									<table class="table table-striped table-bordered datatable-select-inputs nowrap" cellspacing="0" width="100%">
            										<thead>
            											<tr>
            												...
            												<th id="filterable"><center>Arahan RUPS</center></th>
            										...
            										<tbody>
            											<?php $no='1';?>
            											@if(count($arahan_rups))
            											@foreach($arahan_rups as $reason)

            											<tr>
            												<td><center>{{ $no }}</center></td>
            												<td>{{ $reason->arahan_rups }}</td>

            												<td><center>
            													<a href="#" class="btn btn-outline-info btn-sm" data-target="#ubah{{$reason->id}}" data-toggle="modal"><i class="fa fa-edit"></i> Edit</a>
            													<a href="#" class="btn btn-danger btn-sm" data-target="#hapus{{$reason->id}}" data-toggle="modal"><i class="fa fa-trash"></i> Hapus</a>
            													...
            													<div class="modal fade" data-backdrop="static" id="hapus{{$reason->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            														<div class="modal-dialog">
            															<div class="modal-content">
            																...
            																<div class="modal-body">
            																	<h4>Anda yakin ingin menghapus arahan RUPS <br><span class=text-danger>{{ $reason->arahan_rups }}</span> ?</h4>
            																</div>
            																<div class="modal-footer">
            																	<a href="{{ URL('arahan_rups/delete_arahan_rups/'. $reason->id) }}"" class="btn btn-success btn-sm"><i class="fa fa-check"></i> Ya</a>
            											...
            											<div class="modal fade" data-backdrop="static" id="ubah{{$reason->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            												<div class="modal-dialog">
            													<div class="modal-content">
            														<div class="modal-header">
            															<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
            															<center><h4 class="modal-title text-info" id="myModalLabel" ><i class="fa fa-edit"></i> Ubah Arahan RUPS</h4></center>
            														</div>
            														<div class="modal-body">
            															<form enctype="multipart/form-data" role="form" action="{{ URL('arahan_rups/update_arahan_rups/'. $reason->id) }}" method="POST" >
            																{{ csrf_field() }}
            																...
            																<label class="control-label"> : </label>
            																<input class="form-control" type="text" name="arahan_rups" placeholder="masukkan arahan RUPS" value="{{$reason->arahan_rups}}" required="required"/>
            															</div>
            															<div class="modal-footer">
